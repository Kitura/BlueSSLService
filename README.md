...
## Overview
SSL Add-on framework for [BlueSocket](https://github.com/IBM-Swift/BlueSocket.git) in Swift using the Swift Package Manager. Works on macOS (using Secure Transport) and Linux (using OpenSSL).
...
- `init(withChainFilePath chainFilePath: String? = nil, usingSelfSignedCerts selfSigned: Bool = true, withPassword password: String? = nil)` - This API allow you to create a configuration using a single `Certificate Chain File` (see note 2 below). Set the second parameter to true if the certificates are `self-signed`, otherwise set it to false. Add an optional password (if required) using the third parameter.
